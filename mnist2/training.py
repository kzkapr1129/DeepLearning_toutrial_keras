...
CNN(Convolutional Neural Network)とは？
  ...
  全結合層のみでは画像中の全ピクセルを学習で使用するのに対して、畳み込み層は画像中のエッジを全結合層の入力に使用するため正解率の向上が期待できる。
  畳み込みはConv2D()とMaxPooling2D()のセットで使われ、Conv2D()でフィルター処理をして、MaxPooling2D()でN*Nピクセルを1ピクセルにして出力しすることで効率的にデータ削減・学習を行う。
  欠点は全結合層のみの場合と比較して習時間が大量に必要になること。
